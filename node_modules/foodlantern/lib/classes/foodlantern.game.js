...
(function () {
    ...
    Game = function (gameData) {
        ...
        this.initialize = function (gameData) {
            ...
            FoodLantern.notify('Initializing game ' + gameData.name);
            
            models.recipes    = require('../models/foodlantern.recipes');
            models.recipes.setParent(self);
            models.lanterns   = require('../models/foodlantern.lanterns');
            models.lanterns.setParent(self);
            models.resources  = require('../models/foodlantern.resources');
        ...
        this.loadLanterns = function () {
            ...
            FoodLantern.notify('Loading lanterns for game ' + self.name);
            // Search the database for any lanterns with the same game_id reference as this game
            FoodLantern.MongoServer.models.lantern.find({game_id: self._id}, function (err, lanternList) {
                FoodLantern.notify('Found ' + lanternList.length + ' lanterns for game ' + self.name);
                if (err) {
                    deferred.reject(err);
                } else {
        ...
        this.loadRecipes = function () {
            var deferred = Q.defer();
            FoodLantern.notify('Loading recipes for game ' + self.name);
            FoodLantern.MongoServer.models.recipe.find({game_id: self._id}, function (err, recipeList) {
                FoodLantern.notify('Found ' + recipeList.length + ' recipes for game ' + self.name);
                if (err) {
                    deferred.reject(err);
                } else {
                    models.recipes.loadAll(recipeList)
                        .then(function () {
                            FoodLantern.notify('Finished loading ' + self.recipes.length + ' recipe(s) for game ' + self.name);
                            deferred.resolve();
                        }, console.log);
                }
            });
        ...
        this.loadResources = function () {
            var deferred = Q.defer();
            FoodLantern.notify('Loading all resource types');
            FoodLantern.MongoServer.models.resource.find({}, function (err, resourceList) {
                FoodLantern.notify('Found ' + resourceList.length + ' resource types on the server.');
                if (err) {
                    deferred.reject(err);
                } else {
        ...
        this.save = function () {
            ...
            console.log('Saving Game: ' + self._id, newData);
